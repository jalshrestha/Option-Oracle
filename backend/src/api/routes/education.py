...
async def get_educational_content(
    ...
    try:
        ...
        if topic:
            stmt = stmt.where(EducationalContent.topic == topic)
        if difficulty:
            stmt = stmt.where(EducationalContent.difficulty == difficulty)
        if content_type:
            stmt = stmt.where(EducationalContent.content_type == content_type)
        stmt = stmt.order_by(EducationalContent.created_at.desc()).limit(limit)
