    ...
    async def _execute_paper_trade(
        ...
        try:
            ...
            if recommendation.option_type:
                # Options trade
                # OCC symbology: SYMBOL(6) + YYMMDD(6) + C/P(1) + strike*1000 zero-padded to 8
                # e.g. AAPL  250321C00150000 for AAPL $150 call expiring 2025-03-21
                _yymmdd = recommendation.expiration_date.replace('-', '')[2:]  # strip century YYYY→YY
                option_symbol = f"{symbol:<6}{_yymmdd}{recommendation.option_type[0].upper()}{int(recommendation.strike_price * 1000):08d}"
